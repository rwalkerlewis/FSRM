...
TEST_F(MMSElasticityTest, StressTensorMatchesAnalyticalHooke) {
    ...
    // Note: GeomechanicsKernel::residual() implements a strong-form placeholder
    // (stress row sums), not a weak-form FEM divergence. The residual is nonzero
    // because this standalone kernel is never used in the actual PetscDS FEM assembly.
    // The FEM assembly uses PetscFEElasticity f0/f1/g3 callbacks (verified by
    // Physics.ElastostaticsPatch). Just verify the residual is finite.
    EXPECT_TRUE(std::isfinite(static_cast<double>(f[0])));
