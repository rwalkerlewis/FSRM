#!/usr/bin/env python3
"""
Visualize FSRM C++ Simulator Output for the DPRK 2017 Punggye-ri Nuclear Test Model

Reads output from the underground_explosion_near_field example (run with DPRK 2017 parameters)
and produces publication-quality figures showing full-wavefield results.

Usage:
    python scripts/visualize_fsrm_dprk.py

Related scripts:
    visualize_fsrm_lop_nor.py  — Lop Nor 2020 decoupled test visualization
"""
...
OUTDIR  = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                       "figures", "dprk_2017")
PREFIX  = "explosion_near_field_dprk"

# DPRK 2017 Punggye-ri parameters
DEPTH_M = 760
CAVITY_R = 280
ZONE_CRUSHED = 700
ZONE_FRACTURED = 1400
...
def fig_fsrm_source_time_function():
    ...
    fig.suptitle("FSRM Simulator: Mueller-Murphy Source — DPRK 2017 Punggye-ri\n"
...
def fig_fsrm_ground_motion():
    ...
    # Mark zone boundaries: 280, 700, 1400, 2800 m
    for r, lab, col in [(CAVITY_R, "Cavity", "k"), (ZONE_CRUSHED, "Crushed", "r"),
                         (ZONE_FRACTURED, "Fractured", "orange"), (ZONE_DAMAGED, "Damaged", "gold")]:
    ...
    fig.suptitle("FSRM Simulator: Near-Field Ground Motion — 250 kt at 760 m Depth\n"
                 "DPRK 2017 Punggye-ri Granite/Syenite (Vp=5600 m/s)",
                 fontsize=13, fontweight="bold")
    plt.tight_layout(rect=[0, 0, 1, 0.94])
...
def fig_fsrm_damage_map():
    ...
    # Columns: x, z, damage, pressure, velocity
    x = data[:, 0]
    z = data[:, 1]
    D = data[:, 2]
    P = data[:, 3]
    ...
    ax.set_aspect("equal")
    # Mark cavity at (0, -760) with radius 280 m
    ax.add_patch(Circle((0, -DEPTH_M), CAVITY_R, fill=False, ec="cyan", lw=2, ls="--"))
    ax.plot(0, -DEPTH_M, "c*", ms=12)
    ...
    fig.suptitle("FSRM Simulator: Near-Field Damage & Pressure — DPRK 2017 Punggye-ri\n"
                 "Cross-section through ground zero",
                 fontsize=13, fontweight="bold")
    plt.tight_layout(rect=[0, 0, 1, 0.94])
...
def fig_fsrm_time_history():
    ...
    fig.suptitle("FSRM Simulator: Time History — DPRK 2017 Punggye-ri\n"
                 "Cavity evolution & seismic source function",
                 fontsize=13, fontweight="bold")
    plt.tight_layout(rect=[0, 0, 1, 0.94])
...
def fig_fsrm_combined_summary():
    ...
    lines = [
        ...
        "  Depth:    760 m",
        "  Host:     Granite/syenite (Vp=5600 m/s)",
        "  Cavity radius:  280 m",
        "  Crushed zone:   700 m",
        "  Fractured zone: 1400 m",
        "  Damaged zone:   2800 m",
        "",
        "Results:",
        "  mb (coupled):   5.80",
    ...
    fig.suptitle("FSRM C++ Full-Wavefield Simulation Summary\n"
                 "250 kt Fully Coupled Underground Nuclear Test — DPRK 2017 Punggye-ri",
...
def main():
    ...
    print("  DPRK 2017 Punggye-ri Nuclear Test Model")
    print("=" * 60)
    print()
