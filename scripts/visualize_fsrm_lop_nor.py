#!/usr/bin/env python3
"""
Visualize FSRM C++ Simulator Output for the Lop Nor 2020 Nuclear Test Model
...
def fig_fsrm_source_time_function():
    ...
    fig.suptitle("FSRM Simulator: Mueller-Murphy Source — 1 kt Decoupled Test\n"
...
def fig_fsrm_ground_motion():
    ...
    fig.suptitle("FSRM Simulator: Near-Field Ground Motion — 1 kt at 1 km Depth\n"
                 "Lop Nor Granite (Vp=5500 m/s, ρ=2650 kg/m³)",
                 fontsize=13, fontweight="bold")
    plt.tight_layout(rect=[0, 0, 1, 0.94])
...
def fig_fsrm_damage_map():
    ...
    ax.set_aspect("equal")
    # Mark cavity
    ax.add_patch(Circle((0, -1000), 55, fill=False, ec="cyan", lw=2, ls="--"))
    ax.plot(0, -1000, "c*", ms=12)
    ...
    fig.suptitle("FSRM Simulator: Near-Field Damage & Pressure — 1 kt Decoupled Test\n"
...
def fig_fsrm_time_history():
    ...
    fig.suptitle("FSRM Simulator: Time History — 1 kt Decoupled Test at Lop Nor\n"
...
def fig_fsrm_combined_summary():
    ...
    fig.suptitle("FSRM C++ Full-Wavefield Simulation Summary\n"
                 "1 kt Decoupled Underground Nuclear Test — Lop Nor Granite",
