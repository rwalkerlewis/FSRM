...
OUTPUT_DIR = os.path.join(PROJECT_ROOT,
                          "output", "dprk_2017_far_field", "seismograms")
...
DX = 500.0            # m
VP_MAX = 8050.0       # m/s  (upper-mantle Pn beneath Korean Peninsula)
DOMAIN_KM = 200.0     # km  (lateral extent)
CFL_TARGET = 0.4
DT_SUGGESTED = 0.02   # s  (from config)
NPROCS_MIN, NPROCS_MAX = 16, 64
BYTES_PER_DOF = 8     # double precision
DOF_PER_CELL = 9      # stress(6) + velocity(3)
DURATION = 300.0       # s  simulation time
...
def validate_config():
    ...
    wall_16 = wall_serial / NPROCS_MIN
    wall_64 = wall_serial / NPROCS_MAX
    print(f"  Simulation time   : {DURATION:.0f} s  →  ~{n_steps:,} time-steps")
    print(f"  Throughput est.   : {THROUGHPUT:.0e} cell·steps/core/s")
    print(f"  Wall-clock ({NPROCS_MIN:2d} cores): ~{wall_16/3600:.1f} h")
    print(f"  Wall-clock ({NPROCS_MAX:2d} cores): ~{wall_64/3600:.1f} h")
    print()
...
def print_invocation(nprocs=32):
...
def plot_numerical_record_section(traces, title_extra=""):
    ...
    ax.set_title(f"FSRM Numerical Record Section — DPRK 2017 Far-Field{title_extra}",
...
def compare_numerical_vs_analytical(numerical_traces, analytical_model,
    ...
    fig.suptitle("FSRM Numerical vs Analytical (Mueller-Murphy) — DPRK 2017\n"
                 "250 kt coupled, Punggye-ri 1-D velocity model",
                 fontsize=13, fontweight="bold")
...
def extrapolate_to_far_field(traces, true_distances_km, domain_distances_km,
    ...
    """
    from obspy import Trace as ObspyTrace
    import copy

    corrected = []
    Vp_avg = 6.5  # km/s average crustal P velocity
...
def fig_spectral_comparison(traces):
    ...
    fig.suptitle("Spectral Comparison — FSRM Numerical vs Analytical\n"
                 "DPRK 2017, 250 kt coupled, Punggye-ri model",
                 fontsize=13, fontweight="bold")
